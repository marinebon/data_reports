...
```{R}
#| code-summary: sankey diagram of regional programs to IOOS RAs
data <- data %>%
  mutate(
    ...
    # web_link = ifelse(tolower(web_link) %in% no_values, "", web_link),
    # web_link = ifelse(tolower(web_link) %in% yes_values, "on www", web_link)
  )
